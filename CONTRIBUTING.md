...
pip install pytest pyyaml ruff mypy yamllint google-yamlfmt
```

The Dockerfile linters `hadolint` and `dockerfmt` ship as pre-built binaries on their GitHub release pages; `.github/workflows/lint.yaml` shows the exact `curl` commands to install them.
