...
EOF
    for entry in "${data[@]}"; do
        ...
        echo "<tr><td>$r_time</td><td>$r_path</td><td class='val-text'>$r_lat</td><td class='val-text'>$r_thr</td><td class='status-warn'>[Contact Support]</td></tr>" >> "$HTML_FILE"
    done
...
save_lspci_info() {
    ...
    local TOPOLOGY_LOG="${OUTPUT_P2P}/lspci-topology_${label}.log"
    local VVV_LOG="${OUTPUT_P2P}/lspci-vvv_${label}.log"

    echo -e "${BLUE}[$(date +%T)] Saving lspci info for: $label${NC}" | tee -a "$LOG_FILE"

    lspci -tv > "$TOPOLOGY_LOG" || echo "lspci -tv failed" >> "$LOG_FILE"
    lspci -vvv > "$VVV_LOG" || echo "lspci -vvv failed" >> "$LOG_FILE"
}
